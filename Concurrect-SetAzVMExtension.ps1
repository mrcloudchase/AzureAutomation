...
$vmCollection | Foreach-Object -ThrottleLimit 2 -Parallel {
    ...
    elseif ($vmOsType -eq "Linux") {
        ...
        $protectedSettings = "{'storageAccountName': '$storageAccountName', 'storageAccountSasToken': '$sasToken', 'sinksConfig': {'sink': [{'name': 'nixsyslogjsonblob', 'type': 'JsonBlob'}, {'name': 'nixjsonmetricsblob', 'type': 'JsonBlob'}]}}"
